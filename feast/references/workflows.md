...
4. `simulateAutomations` **with your proposed, un-saved `edits`** — dry-run the flow against a synthetic event timeline with **no real sends** and confirm the right automations would fire. This is the required verification step: automations have no draft/staging tier, so the write in step 5 goes straight to production. Simulate first, write once confident; if the simulation surprises you, fix the edits and re-simulate rather than writing and patching live.
   **Don't hand-build the event timeline** — the server seeds it for you:
   - First call: `{ "flowId": "<id>", "edits": [ ...your proposed operations... ] }` and **omit `events`**. The server auto-seeds from the flow's triggers (a `viewCampaign` event when the flow has a campaign, then the first eligible trigger event 15s later) and returns the exact timeline it ran as `eventsUsed`, alongside `scheduledTexts` (what would send).
   - Branch testing and continuations: copy `eventsUsed`, then edit it (change a reply body to test "YES" vs "STOP") or append to it (add a reply event after the checkout, spacing timestamps ~15s apart), and pass the full array back as `events`. There is no append shortcut — you always send the complete timeline you want to run.
   - Either `events` or `flowId` is required. Passing `"events": []` is NOT auto-seed — it means "no events, evaluate at current time".
   - If the CLI rejects `flowId` as an unknown field or insists `events` is required, the server predates auto-seeding — fall back to constructing `events` by hand from the schema (`describe simulateAutomations`).
